...
public class CustomerView extends JPanel 
{
	 ...
	 * 인자	: 없음
	 * 리턴값	: void
	 * 역할	: 입력한 전화번호를 넘겨주고 DB에서 고객정보를 리턴 받은 후 텍스트필드에 출력
	 */
	 ...
	 * 인자	: 없음
	 * 리턴값	: void
	 * 역할	: 입력한 이름에 해당해는 전화번호를 검색한 메세지를 띄우고 선택한 전화번호를 가진 고객의 정보를 텍스트필드에 출력
	 */
	public void searchByName() {
		...
		try {
			...
			ArrayList<String> list = model.selectByName(name);	// 전화번호 저장
			Object[] obj = list.toArray();	// 오브젝트 업캐스팅
			...
			// 5. 넘겨받은 Customer의 각각의 값을 화면 텍스트 필드 출력
			tfCustTel.setText(vo.getTel());
			tfCustTelAid.setText(vo.getAddTel());
			tfCustName.setText(vo.getName());
			tfCustAddr.setText(vo.getAddr());
	...
	public void updateCustomer(){
		...
		// 3. Model 클래스 안에 updateCustomer () 메소드 호출하여 2번 VO 객체를 넘김
		try {
